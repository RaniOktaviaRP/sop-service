...
export class DivisionsController {
  ...
  async findAll(): Promise<ApiResponseType<Division[]>> {
    const divisions = await this.divisionsService.findAll();
    return {
      data: divisions,
  ...
  @Get('dropdown')
  @Public()
  @ApiOperation({ summary: 'Ambil data divisi untuk dropdown' })
  @ApiResponse({ status: 200, description: 'Berhasil mengambil data dropdown' })
  async getDropdown(): Promise<ApiResponseType<{ division_name: string }[]>> {
    const divisions = await this.divisionsService.findAll();
    return {
  ...
  async create(
    ...
    const division = await this.divisionsService.create(data);
    return {
      data: division,
  ...
  async patch(
    ...
    const updated = await this.divisionsService.update(id, data);
    return {
      data: updated,
